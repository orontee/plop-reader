...
static auto replaceAll(std::string subject, const std::string &search,
...
auto Log::logWithLevel(unsigned int level, const char *str...) -> int {
  ...
  snprintf(outerBuffer, sizeof(outerBuffer), "[%s][%s] %s<br>\n", time_buffer,
           levelsStrings[level], escaped.c_str());
