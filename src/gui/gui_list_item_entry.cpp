...
void GuiListItemEntry::draw(bool clearBeforeDraw, bool mustUpdateScreen,
                            bool highlight) {
  char buffer[2048];
  int yy = y;
  int initialY = y;
  ...
  snprintf(buffer, sizeof(buffer), "%dmin",
           (int)ceil((float)entry.reading_time / (float)60));
  DrawString(xInfos, yy - 2, buffer);
  xInfos += StringWidth(buffer);
  xInfos += 12;
  ...
  // snprintf(buffer, sizeof(buffer), "#%d/%s a%d/%d *%d/%d u%d/%d t%d %s",
  // entry.id, entry.remote_id.c_str(), entry.local_is_archived,
  // entry.remote_is_archived, entry.local_is_starred, entry.remote_is_starred,
  // entry.local_updated_at, entry.remote_updated_at,
  // (int)ceil((float)entry.reading_time/(float)60),
  // (entry.local_content_file_epub != "" ? "E" : "H")); DrawString(textX + 100,
  // yy, buffer);
