...
// There are things (like opening a font?) it seems we cannot do in the ctor:
// the ereader is not ready yet... So, we need another initialization method,
// called later -- like when the ereader is ready to let us use its API...
void Gui::init() {
  OpenScreen();
  iv_fullscreen();

  // Force a black-flash, to fully redraw the screen -- so we don't see Home
  // screen anymore
  FullUpdate();
...
void Gui::show(int numPage, int numberOfPages, int countAllEntries,
  ...
  if (mode == 1) {
    snprintf(buffer, sizeof(buffer), PLOP_APPLICATION_FULLNAME " - %s",
             LBL_HEADER_UNREAD_ENTRIES);
  } else if (mode == 2) {
    snprintf(buffer, sizeof(buffer), PLOP_APPLICATION_FULLNAME " - %s",
             LBL_HEADER_ARCHIVED_ENTRIES);
  } else if (mode == 3) {
    snprintf(buffer, sizeof(buffer), PLOP_APPLICATION_FULLNAME " - %s",
             LBL_HEADER_STARRED_ENTRIES);
  } else {
  ...
  snprintf(buffer, sizeof(buffer), LBL_HEADER_PAGE_NUM, numPage, numberOfPages,
           countAllEntries);
  DrawString(90, y, buffer);
  y += 35;
  ...
  for (unsigned int i = 0; i < entriesItems.size(); i++) {
    ...
    if (i < entriesItems.size() - 1 && i < entries.size()) {
      // Draw separator at the bottom of the entry, except for the last one (the
      // status bar has its own separator at the top)
      const int heightSeparator = 2;
      const int ySeparator =
          y + i * item.getHeight() + item.getHeight() - heightSeparator;
...
// This needs to be global, so it can be accessed by the callback function of
// the menu ;-(
static imenuex *menu;
...
void Gui::displayMainMenu() {
  ...
  auto callback = [](int index) {
    ...
    if (index == 1) {
      ...
      const char *text = PLOP_APPLICATION_FULLNAME
          " " PLOP_VERSION_STR "\n" PLOP_WEBSITE_URL "\n"
          "\n"
          "%1$s\n"
          "\n"
      ...
      char buffer[2048];
      snprintf(buffer, sizeof(buffer), text, LBL_ABOUT_MAIN_DESCRIPTION,
               LBL_ABOUT_DEVELOPED_BY_PM, LBL_ABOUT_CONTRIBUTE);

      DialogSynchro(ICON_INFORMATION, PLOP_APPLICATION_FULLNAME, buffer, "OK",
                    NULL, NULL);

      app.getGui().statusBarText(LBL_STATUSBAR_FEEL_FREE_TO_CONTRIBUTE,
                                 PLOP_OPENSOURCE_URL);

      DEBUG("Opening About dialog - closed");
    } else if (index == 5) {
      int result = DialogSynchro(ICON_QUESTION, LBL_DELETEALL_DIALOG_TITLE,
                                 LBL_DELETEALL_DIALOG_CONTENT,
                                 LBL_DELETEALL_DIALOG_BTN_OK,
                                 LBL_DELETEALL_DIALOG_BTN_CANCEL, NULL);
      if (result == 1) {
  ...
  OpenMenuEx(menu, 0, (screenWidth - rect.w) / 2, (screenHeight - rect.h) / 3,
             callback);
    ...
    for (unsigned int i = 0; i < entriesItems.size(); i++) {
      ...
      if (item.hit(x, y)) {
        if (item.hasEntry()) {
          ...
          statusBarText(LBL_STATUSBAR_LOADING_READER_FOR_ENTRY,
                        item.getEntry().id, item.getEntry().title.c_str());
  ...
  for (unsigned int i = 0; i < entriesItems.size(); i++) {
    ...
    if (item.hit(x, y)) {
      if (item.hasEntry()) {
        ...
        statusBarText(LBL_STATUSBAR_OPENING_CONTEXTMENU_FOR_ENTRY,
                      item.getEntry().id, item.getEntry().title.c_str());
...
static void contextEntryOnMenuHandler(int index) {
  ...
  if (index == 1) {
    app.getGui().statusBarText(LBL_STATUSBAR_OPENING_READERAPP_FOR_ENTRY_FORMAT,
                               contextMenuEntry.id,
                               contextMenuEntry.title.c_str(), "HTML");
    app.read(contextMenuEntry, Application::FORMAT_HTML);
  } else if (index == 2) {
    app.getGui().statusBarText(LBL_STATUSBAR_OPENING_READERAPP_FOR_ENTRY_FORMAT,
                               contextMenuEntry.id,
                               contextMenuEntry.title.c_str(), "EPUB");
...
void Gui::displayContextMenuOnEntry(GuiListItemEntry &item, int xTouch,
                                    int yTouch) {
...
void Gui::displayHelpWhenNoLocalData() {
  ...
  DrawString(150, screenHeight - 280 + 50 + 30 + 30,
             LBL_ONBOARDING_STATUSBAR_03);

  DrawLine(250, screenHeight - 280 + 50 + 30 + 30 + 30, 450, screenHeight - 50,
           BLACK);
...
void Gui::openProgressBar(int icon, const char *title, const char *text,
                          int percent, iv_dialoghandler hproc) {
  OpenProgressbar(icon, title, text, percent, hproc);
}

auto Gui::updateProgressBar(const char *text, int percent) -> int {
  return UpdateProgressbar(text, percent);
}

void Gui::closeProgressBar() { CloseProgressbar(); }
