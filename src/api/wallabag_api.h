...
public:
  ...
  void
  loadRecentArticles(EntryRepository repository,
  ...
  void
  enqueueEpubDownload(EntryRepository &repository, Entry &entry,
                      EpubDownloadQueueRepository &epubDownloadQueueRepository,
  ...
  void downloadEpub(EntryRepository &repository, Entry &entry,
                    gui_update_progressbar progressbarUpdater, int percent);
...
private:
  ...
  static auto _curlWriteCallback(char *ptr, size_t size, size_t nmemb,
  ...
  CURLcode doHttpRequest(
      ...
      std::function<void(CURLcode res, long response_code, CURL *curl)>
          onFailure,
