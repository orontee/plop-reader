...
auto WallabagEntitiesFactory::datetimeStringToTimeT(const char *str) -> time_t {
  struct tm tm;
...
auto WallabagEntitiesFactory::createEntryFromJson(json_object *item) -> Entry {
  int id = json_object_get_int(json_object_object_get(item, "id"));
  int is_archived =
      json_object_get_int(json_object_object_get(item, "is_archived"));
  int is_starred =
      json_object_get_int(json_object_object_get(item, "is_starred"));
  const char *title =
  ...
  const char *content =
      json_object_get_string(json_object_object_get(item, "content"));
  const char *created_at_str =
      json_object_get_string(json_object_object_get(item, "created_at"));
  const char *updated_at_str =
      json_object_get_string(json_object_object_get(item, "updated_at"));
  int reading_time =
      json_object_get_int(json_object_object_get(item, "reading_time")) * 60;
  const char *preview_picture =
      json_object_get_string(json_object_object_get(item, "preview_picture"));
  ...
  // When creating an entry locally, its local values are the same as remote
  // ones
  ...
  entry.title = title != nullptr ? title : std::string();
  entry.url = url != nullptr ? url : std::string();
  entry.content = content != nullptr ? content : std::string();
  ...
  entry.preview_picture_url =
      preview_picture != nullptr ? preview_picture : std::string();
...
auto WallabagEntitiesFactory::mergeLocalAndRemoteEntries(Entry &local,
                                                         Entry &remote)
    -> Entry {
  Entry entry;
  ...
  if (remote.remote_updated_at > local.local_updated_at &&
      local.local_is_archived != remote.remote_is_archived) {
  ...
  if (remote.remote_updated_at > local.local_updated_at &&
      local.local_is_starred != remote.remote_is_starred) {
  ...
  // For some other fields that are more specific to the application, we always
  // use what we had locally
  ...
  if (isLocalUpdated && remote.remote_updated_at > local.local_updated_at) {
    ...
    // AND the entry has been updated more recently on the server than on the
    // device
    // => use the updated_at date from the server
    entry.local_updated_at = remote.remote_updated_at;
    entry._isChanged = true;
  } else {
    // Entry has not been updated on the server, or it's more recent on the
    // ereader
