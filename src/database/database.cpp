...
void Database::open() {
  if (sqlite3_open(Database::DB_FILE, &this->db)) {
    ERROR("Fail opening database file '%s': %s", Database::DB_FILE,
...
void Database::runMigrations() {
  ...
  DEBUG("Current migrationVersion of DB = %s ; targetVersion = %s",
        migrationVersion.value.c_str(), currentVersion.c_str());
...
auto Database::selectInternal(std::string key) -> Internal {
  ...
  if (sqlite3_prepare(this->db, sqlSelect, -1, &stmt, &tail) != SQLITE_OK) {
    ...
    // sqlite3_errmsg(this->db)); log_message(buffer);
  }
  if (sqlite3_bind_text(stmt, sqlite3_bind_parameter_index(stmt, ":key"),
...
void Database::insertInternal(std::string key, std::string value) {
  const char *sqlInsert =
      "insert into internals (`key`, `value`, `created_at`, `updated_at`) "
      "values (:key, :value, strftime('%s','now'), strftime('%s','now'))";
  ...
  if (sqlite3_bind_text(stmt, sqlite3_bind_parameter_index(stmt, ":value"),
  ...
  if (sqlite3_step(stmt) != SQLITE_DONE) {
    // snprintf(buffer, sizeof(buffer), "Fail inserting : %s",
    // sqlite3_errmsg(this->db)); log_message(buffer);
  }
  sqlite3_finalize(stmt);
}
...
void Database::updateInternal(std::string key, std::string value) {
  const char *sqlUpdate = "update internals set value = :value, updated_at = "
                          "strftime('%s','now') where key = :key";
  ...
  if (sqlite3_step(stmt) != SQLITE_DONE) {
    // snprintf(buffer, sizeof(buffer), "Fail updating : %s",
    // sqlite3_errmsg(this->db)); log_message(buffer);
  }
  sqlite3_finalize(stmt);
}
